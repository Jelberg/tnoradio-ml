...
This middleware creates a single WSGI app that dispatches to
multiple other WSGI apps mounted at different URL paths.

A common example is writing a Single Page app, where you have a
...
serving files or proxying to app servers based on location. The
...
class DispatcherMiddleware:
    """Combine multiple apps as a single WSGI app.
    Requests are dispatched to an app based on the path it is
    mounted under.

    :param app: The WSGI app to dispatch to if the request
        doesn't match a mounted path.
    :param mounts: Maps path prefixes to apps for dispatching.
    """

    def __init__(
        self,
        app: "WSGIapp",
        mounts: t.Optional[t.Dict[str, "WSGIapp"]] = None,
    ) -> None:
        self.app = app
