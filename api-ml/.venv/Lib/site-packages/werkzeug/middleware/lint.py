...
behavior of the WSGI server and app. It checks that the
...
class InputStream:
    ...
    def close(self) -> None:
        warn("The app closed the input stream!", WSGIWarning, stacklevel=2)
        self._stream.close()
...
class ErrorStream:
    ...
    def close(self) -> None:
        warn("The app closed the error stream!", WSGIWarning, stacklevel=2)
        self._stream.close()
...
class GuardedIterator:
    ...
    def __next__(self) -> bytes:
        ...
        if not self.headers_set:
            warn(
                "The app returned before it started the response.",
                WSGIWarning,
                stacklevel=2,
            )

        check_type("app iterator items", rv, bytes)
...
class LintMiddleware:
    ...
    server and wrapped app. Some of the issues it checks are:

    -   invalid status codes
    -   non-bytes sent to the WSGI server
    -   strings returned from the WSGI app
    ...
    def check_iterator(self, app_iter: t.Iterable[bytes]) -> None:
        if isinstance(app_iter, bytes):
            warn(
                "The app returned a bytestring. The response will send one"
    ...
    def __call__(self, *args: t.Any, **kwargs: t.Any) -> t.Iterable[bytes]:
        ...
        # Hook our own file wrapper in so that apps will always
