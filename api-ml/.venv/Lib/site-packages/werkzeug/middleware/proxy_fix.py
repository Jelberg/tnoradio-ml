...
``X-Forwarded-`` headers that proxies in front of an app may
set.

When an app is running behind a proxy server, WSGI may see the
...
This middleware should only be used if the app is actually
...
class ProxyFix:
    ...
    def __call__(
        ...
        headers before calling the wrapped app. Store the
