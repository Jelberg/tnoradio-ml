...
Here a simple example that creates a URL map for an app with
...
If the app doesn't use subdomains it's perfectly fine to not set
...
unique identifiers. The WSGI app can use those endpoints to get the
...
.. code-block:: python
    ...
    c = m.bind('example.com', '/apps/example')
...
if t.TYPE_CHECKING:
    ...
    from _typeshed.wsgi import WSGIapp
...
class RoutingException(Exception):
    """Special exceptions that require the app to redirect, notifying
...
class EndpointPrefix(RuleFactory):
    ...
    another string. This can be useful for sub apps::
...
class Rule(RuleFactory):
    ...
    in order to not break the app on Werkzeug upgrades.
    ...
    `subdomain`
        ...
        and all subdomains are forwarded to your app::
    ...
    `build_only`
        ...
        or folder that are not handled by the WSGI app (like static data)
...
class PathConverter(BaseConverter):
    ...
    slashes.  This is useful for wikis and similar apps::
...
class MapAdapter:
    ...
    def dispatch(
        self,
        view_func: t.Callable[[str, t.Mapping[str, t.Any]], "WSGIapp"],
        path_info: t.Optional[str] = None,
        ...
        or WSGI app.  http exceptions are not caught by default
        so that apps can display nicer error messages by just
    ...
    def match(
        ...
        - you receive a `NotFound` exception that indicates that no URL is
          matching.  A `NotFound` exception is also a WSGI app you
          can call to get a default page not found page (happens to be the
          same object as `werkzeug.exceptions.NotFound`)

        - you receive a `MethodNotAllowed` exception that indicates that there
          is a match for this URL but not for the current request method.
          This is useful for RESTful apps.

        - you receive a `RequestRedirect` exception with a `new_url`
          attribute.  This exception is used to notify you about a request
          Werkzeug requests from your WSGI app.  This is for example the
