...
class _TestCookieJar(CookieJar):
    ...
    and to wsgi environments, and wsgi app responses.
    """
...
class EnvironBuilder:
    ...
    :param json: An object to be serialized and assigned to ``data``.
        Defaults the content type to ``"app/json"``.
    ...
    def content_type(self) -> t.Optional[str]:
        ...
        if ct is None and not self._input_stream:
            ...
            if self._form:
                return "app/x-www-form-urlencoded"
            return None
        return ct
    ...
    def get_environ(self) -> "WSGIEnvironment":
        ...
        elif mimetype == "app/x-www-form-urlencoded":
...
class Client:
    """This class allows you to send requests to a wrapped app.

    The use_cookies parameter indicates whether cookies should be stored and
    sent for subsequent requests. This is True by default, but passing False
    will disable this behaviour.

    If you want to request some subdomain of your app you may set
    ...
    def run_wsgi_app(
        ...
        rv = run_wsgi_app(self.app, environ, buffered=buffered)
    ...
    def open(
        ...
        **kwargs: t.Any,
        ...
        request to the app using it, and return the response.
...
def run_wsgi_app(
    app: "WSGIapp", environ: "WSGIEnvironment", buffered: bool = False
) -> t.Tuple[t.Iterable[bytes], str, Headers]:
    """Return a tuple in the form (app_iter, status, headers) of the
    app output.  This works best if you pass it an app that
    returns an iterator all the time.

    Sometimes apps may use the `write()` callable returned
    by the `start_response` function.  This tries to resolve such edge
    cases automatically.  But if you don't get the expected output you
    should set `buffered` to `True` which enforces buffering.

    If passed an invalid WSGI app the behavior of this function is
    undefined.  Never pass non-conforming WSGI apps to this function.
    ...
    # app iterator into a regular list
    if buffered:
        try:
    ...
    # otherwise we iterate the app iter until we have a response, chain
...
class TestResponse(Response):
    ...
    If the test request included large files, or if the app is
